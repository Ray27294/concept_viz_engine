...
def _get_attributes_info(table: TableMetadata, selected_col_names: List[str]):
    """Analyse the selected columns of a table to determine the number of scalar and discrete attributes, 
    ...
    num_discrete = sum(1 for a in attributes if a.semantic_type == "discrete")
    
    is_geo = table.table_name.lower() in GEO_NAMES or any(k.lower() in GEO_NAMES for k in key_names)
    
    return num_scalars, num_discrete, is_geo
...
def recommend_components(
    ...
    num_scalars, num_discrete, is_geo = _get_attributes_info(table, selected_col_names)
    ...
    if "Basic Entity" in pattern:
        ...
        if num_scalars == 1 and num_discrete == 1:
            combinations.append(ChartCombination(chart_name="Histogram", geom="bar", stat="bin"))
            combinations.append(ChartCombination(chart_name="Frequency Polygon", geom="line", stat="bin"))
            combinations.append(ChartCombination(chart_name="Density Plot", geom="density", stat="density"))
        if num_scalars == 2 and num_discrete == 0:
            combinations.append(ChartCombination(chart_name="Scatter Diagram", geom="point", stat="identity"))
        if num_scalars in [3, 4] and num_discrete == 0:
